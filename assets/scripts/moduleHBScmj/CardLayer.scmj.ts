...
export default class CardLayer extends BaseComponent {
    ...
    selfQueType = -1 //-1不提示；0:条；1：万；2：筒;
    typeTiao = 0
	typeWan = 1
	typeTong = 2
    ...
    resetView() {
        ...
        for (let chairId = 0; chairId < GameLogic.Instance().MAX_SEAT_NUM; chairId++) {
            ...
            //四川麻将胡的牌
            this['nodeHuCard' + chairId].active = true
            for (let i = 1; i < 10+1; i++) {
                this['nodeHuCard' + chairId].getChildByName(i.toString()).active = false
                /*if (chairId == 1) {
                    this['handCardFront' + chairId].getChildByName(i.toString()).getChildByName('value').angle = -90
                }*/
            }
    ...
    onTouchBegin(event) {
        ...
        if (selectedCardNode) {
            ...
            if(selectedCardNode.bMask){
                GameLogic.Instance().gamescene.showQueTip("缺还未打完，请先把缺打完",4)
                return
            }
    ...
    refreshHandCard(chairId, vecCards, cHuPai, curCard?) {
        cc.log("refreshHandCard",chairId,vecCards,cHuPai)
        if (!vecCards) {
        ...
        for (let i = 0; i < vecCards.length; i++) {
            ...
            if (card > 0) {
                ...
                if (chairId == 0 && cHuPai != 1) {
                    ...
                    // if (chairId == 0 && this.selfQueType >= 0) {
                    //     this.addQueTipMask(cardNodeHand, card, this.selfQueType)
                    // }
                } else {
        ...
        for(let cardNode of this.handCardList){
            this.addQueTipMask(cardNode, cardNode.cardValue, this.selfQueType)
        }

        if (value) {
            this.refreshCurCard(chairId, value, cHuPai)
        }else if(cHuPai && curCard){
            this.refreshCurCard(chairId, curCard, cHuPai)
        }
    }

    refreshCurCard(chairId, value, cHuPai) {
        cc.log("refreshCurCard chairId=",chairId)
        cc.log("refreshCurCard cHuPai=",cHuPai)
        ...
        if (value > 0) {
            ...
            if (chairId == 0 && cHuPai != 1) {
                ...
                if (chairId == 0 && this.selfQueType >= 0) {
                    this.addQueTipMask(cardNodeHand, value, this.selfQueType)
                    for(let cardNode of this.handCardList){
                        this.addQueTipMask(cardNode, cardNode.cardValue, this.selfQueType)
                    }
                }
    ...
    refreshHuCards(chairId, huCards) {
        cc.log("refreshHuCards",chairId,huCards)
        for (let i = 0; i < huCards.length; i++) {
            let v = huCards[i]
            let cardNode = this['nodeHuCard' + chairId].getChildByName((i+1).toString())
            this.setCardNodeValue(cardNode, v.value)
            this.addMaskColor(cardNode, cc.color(218, 150, 177))

            cardNode.removeChild(cardNode.getChildByName("huNum"))

            if(v.num > 1){
                let jiaobiao = new cc.Node("huNum")
                jiaobiao.setAnchorPoint(cc.v2(0.5,0.5))
                jiaobiao.addComponent(cc.Sprite)
                if(chairId == 0 || chairId == 2){
                    jiaobiao.position = cc.v2(24, 33)
                }else{
                    jiaobiao.position = cc.v2(24, 20)
                }
                jiaobiao.parent = cardNode

                let numLabel = new cc.Node()
                numLabel.setAnchorPoint(cc.v2(0.5,0.5))
                numLabel.addComponent(cc.Label)
                numLabel.position = cc.v2(0, 3)
                numLabel.parent = jiaobiao

                numLabel.getComponent(cc.Label).fontSize = 18
                numLabel.getComponent(cc.Label).lineHeight = 18
                numLabel.getComponent(cc.Label).string = v.num

                BaseFunc.SetFrameTextureLocal(jiaobiao.getComponent(cc.Sprite),"moduleHBScmjRes/images/ZhuJieMian/icon_hongdian_numBg", (sprite) => {
                })
            }
        }
    }
    ...
    moveHandCardsAfterChuPai(chuCardNode) {
        ...
        let moveCard = function (nextCardNode, cardNode) {
            ...
            this.addQueTipMask(nextCardNode, cardNode.cardValue, this.selfQueType)
        ...
        for (let i = chuCardNode.index - 1; i >= 0; i--) {
            ...
            let bmove = false
            let cardQueValue = this.getQueCardValue(cardNode.cardValue)
            let curCardQueValue = this.getQueCardValue(curCradNode.cardValue)
            cc.log("curCardQueValue=",cardQueValue,curCardQueValue)
            if (bCurJin) {
                if (bNodeJin) {
                    if (cardQueValue > curCardQueValue) {
                        bmove = true
                    }
                } else {
                    bmove = true
            ...
            } else if (cardQueValue > curCardQueValue) {
                bmove = true
            }
    ...
    checkJinPai(cardNode) {
        return false //GameLogic.Instance().gamescene.checkJinPai(cardNode.cardValue)
    }
    ...
    getCardType(value){
        if(value >= 1 && value <= 9){
            ...
            return this.typeTong
        } else if(value >= 21 && value <= 29){
            return this.typeTiao
        } else {
            return -1
        }
    }
    ...
    getQueCardValue(cardValue){
        cc.log("getQueCardValue cardValue = ",cardValue)
        let ret = cardValue
        if(this.getCardType(cardValue) == this.selfQueType && this.selfQueType != -1){
            cc.log("getQueCardValue ret = ",ret)
            ret = ret + 100
            cc.log("getQueCardValue ret1 = ",ret)
        }
        return ret
    }

    addQueTipMask(cardNodeHand, cardValue, queType){
        let cardType = this.getCardType(cardValue)
        let haveQueCard = false

        for (let i = 0; i < this.handCardList.length; i++) {
            let cardNode = this.handCardList[i]
            let cardType = this.getCardType(cardNode.cardValue)
            if(cardType == queType){
                haveQueCard = true
            }
        }

        let curCardNodeHand = this.getCurCardNodeHand(0)
        if(curCardNodeHand.active){
            let cardType = this.getCardType(curCardNodeHand.cardValue)
            if(cardType == queType){
                haveQueCard = true
            }
        }
        if(cardType != -1 && cardType != queType && haveQueCard){
            this.addMask(cardNodeHand,true)
        }else{
            this.removeMask(cardNodeHand)
        }
    }
