...
import GamePlayerStateController from "./GamePlayerStateController.scmj"
...
export default class GamePlayer extends GamePlayerStateController {
    ...
    start () {
        this.originalPos = this.sptQue.getPosition()
    }
    ...
    setZhuang(isZhuang) {
        ...
        this.sptZhuang.active = isZhuang
    }
    ...
    setMapaiNum(num) {
        ...
        this.mapaiNum = num
        // this.refreshPlayerInfo(8)
        if (GameLogic.Instance().gametype == 1) {
        // if (self.parentView.logic.gametype == 1) {
        } else { // 马牌
            // if (type == nil or type == 8) {
            this.sptMa.active = this.isZhuang
            // --self.lbl_mapaiNum:setVisible(self.isZhuang)
            // --self.lbl_mapaiNum:setString("x"..self.mapaiNum)
            // }
        }
    }
    // update (dt) {}
