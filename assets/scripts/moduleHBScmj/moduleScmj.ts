...
import proto = require("./proto/client.scmj")
import opcodeConfig from "./proto/opcode.scmj";
...
let gameTexture = [
    ...
    // "/moduleHBErmjRes/images/GameScene/GameScene.scmj",
    "/moduleHBErmjRes/images/MaJiang/MaJiang.hbermj",
    // "/moduleHBErmjRes/images/Operate/OperateStyle2.scmj",
...
export default class ModuleStart extends cc.Component {
    ...
    start () {
        czcEvent(GameLogic.Instance().gameName, "加载1", "开始加载" + GameLogic.Instance().gameName + (DataManager.CommonData["morrow"] <= 1 ? DataManager.CommonData["morrow"] + "天新用户" : "老用户"))
    ...
    setPreload(scene: BaseScene) {
        ...
        preload.startLoad(gameTexture, audioParam, (items) => {
            czcEvent(GameLogic.Instance().gameName, "加载2", "加载完成" + (DataManager.CommonData["morrow"] <= 1 ? DataManager.CommonData["morrow"] + "天新用户" : "老用户"))
            SceneManager.Instance.addScene<String>("moduleHBScmjRes", "GameScene")    
