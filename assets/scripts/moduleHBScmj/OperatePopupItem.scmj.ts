...
export default class OperatePopupItem extends BaseComponent {
    ...
    @property()
    data:[] = []
    thisComponentName = "OperatePopupItem.scmj"
    ...
    init(data) {
        ...
        for (const value of vecCards) {
            GameLogic.Instance().addPrefab(this.prefab_card, this.node).getComponent("Card.scmj").init(value)
        }
    }

    onPressOperate() {
        GameLogic.Instance().gamescene.mahjong_mj_playcard_ack_sender(this.data.opcode, this.data.value, GameLogic.Instance().gamescene.curSerialID)
    }
}