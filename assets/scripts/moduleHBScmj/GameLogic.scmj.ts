...
import AudioManager from "./AudioManager.scmj";

import {MsgBox, gobackToMain, unenoughGold, checkServerMoneyLimit, showShopPop, getGameAward, czcEvent} from "../base/BaseFuncTs";
...
export default class GameLogic extends BaseScene {
    ...
    socketName: string = 'scmj'

    moduleExt: string = 'scmj'

    moduleRes: string = 'moduleHBScmjRes/'
    ...
    analyzeSocketInfo(serverInfo) {
        ...
        if (this.serverInfo.gameId == 395) {
            this.gameName = "血战到底"
        }else if (this.serverInfo.gameId == 396) {
            this.gameName = "血流成河"
        }

        cc.log("analyzeSocketInfo this.gameName", this.gameName)
    }
    ...
    showGameCommonTipLayer(initParam) {       
        initParam = initParam || [] 
        SceneManager.Instance.popScene<String>("moduleHBScmjRes", "GameCommonTipLayer", initParam)
    }

    showGameFanXingTipLayer(initParam = []) { 
        initParam["zorder"] = this.gamescene.zOrder.nodeJieSuan
        SceneManager.Instance.popScene<String>("moduleHBScmjRes", "GameFanXingTipLayer", initParam)
    }
    ...
    showGameResultLayer(initParam = []) { 
        ...
        initParam["zorder"] = this.gamescene.zOrder.nodeJieSuan
        SceneManager.Instance.popScene<String>("moduleHBScmjRes", "GameResultLayer", initParam)
    }
	...
	LeaveGameScene(param = {isOnGameExit: 0}) {
        czcEvent(GameLogic.Instance().gameName, "离桌1", "游戏返回大厅"+ (DataManager.CommonData["morrow"] <= 1 ? DataManager.CommonData["morrow"] + "天新用户" : "老用户"))
        if(this.henry_debug){
            return
        }
        ...
        /*if self.gamePage then
            if self.gametype == 1 then
                self.gamePage.view:clearWind()
            end
            // echoError("111");
            print("GameLogic:LeaveGameScene:",num)
            if (self.gamePage ~= nil) then
                self.is_chang_table = 0;
                if self.gamePage.view then
                    self.gamePage.view.rootNode:stopAllActions()
                end
            end

            self.gamePage.view.m_players[2]:unschedulerMoveScheduler()
            self.gamePage.view.m_players[2]:unschedulerMatchScheduler()
        end*/
