...
import proto = require("./proto/client.gdmj")
import opcodeConfig from "./proto/opcode.gdmj";
...
let gameTexture = [
    ...
    // "/moduleHBErmjRes/images/GameScene/GameScene.gdmj",
    "/moduleHBErmjRes/images/MaJiang/MaJiang.hbermj",
    // "/moduleHBErmjRes/images/Operate/OperateStyle2.gdmj",
...
export default class ModuleStart extends cc.Component {
    ...
    start () {
        czcEvent("广东麻将", "加载1", "开始加载广东麻将" + (DataManager.CommonData["morrow"] <= 1 ? DataManager.CommonData["morrow"] + "天新用户" : "老用户"))
    ...
    setPreload(scene: BaseScene) {
        ...
        preload.startLoad(gameTexture, audioParam, (items) => {
            czcEvent("广东麻将", "加载2", "加载完成" + (DataManager.CommonData["morrow"] <= 1 ? DataManager.CommonData["morrow"] + "天新用户" : "老用户"))
            SceneManager.Instance.addScene<String>("moduleGdmjRes", "GameScene")    
