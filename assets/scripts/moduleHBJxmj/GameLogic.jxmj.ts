...
import AudioManager from "./AudioManager.jxmj";
...
export default class GameLogic extends BaseScene {
    ...
    socketName: string = 'jxmj'

    moduleExt: string = 'jxmj'

    moduleRes: string = 'moduleHBJxmjRes/'
    ...
    showGameCommonTipLayer(initParam) {       
        initParam = initParam || [] 
        SceneManager.Instance.popScene<String>("moduleHBJxmjRes", "GameCommonTipLayer", initParam)
    }

    showGameFanXingTipLayer(initParam = []) { 
        initParam["zorder"] = this.gamescene.zOrder.nodeJieSuan
        SceneManager.Instance.popScene<String>("moduleHBJxmjRes", "GameFanXingTipLayer", initParam)
    }
    ...
    showGameResultLayer(initParam = []) { 
        ...
        initParam["zorder"] = this.gamescene.zOrder.nodeJieSuan
        SceneManager.Instance.popScene<String>("moduleHBJxmjRes", "GameResultLayer", initParam)
    }
	...
	LeaveGameScene(param = {isOnGameExit: 0}) {
        czcEvent("江西红中", "离桌1", "游戏返回大厅"+ (DataManager.CommonData["morrow"] <= 1 ? DataManager.CommonData["morrow"] + "天新用户" : "老用户"))
        if(this.henry_debug){
            return
        }
