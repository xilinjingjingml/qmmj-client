...
import GamePlayerStateController from "./GamePlayerStateController.jxmj"
...
export default class GamePlayer extends GamePlayerStateController {
    ...
    onEnterStartGame() {
        ...
        if (GameLogic.Instance().bEnterInGame == false) {
            GameLogic.Instance().gamescene.playStartGameAni()
        }
    }
    ...
    setPiao(type) {
        ...
        if (sptName != "") {
            cc.log("sptName = "+sptName)
            BaseFunc.SetFrameTextureLocal(this.sptPiao.getComponent(cc.Sprite),"moduleHBJxmjRes/images/ZhuJieMian/"+sptName, (sprite) => {
                this.sptPiao.active = true
			})
        }
    }
}
