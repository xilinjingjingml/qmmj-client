...
export default class NewUserSign extends BaseScene {
    ...
    onOpenScene() {
        ...
        if (null == DataManager.CommonData["NewUserSgin"])
        ...
        DataManager.saveKeyWithDate("newUserSingOpen")
    }
    ...
    initSignRows() {
        ...
        for (const iterator of DataManager.CommonData["NewUserSgin"]["rows"]) {
            ...
            let label1 = day.getChildByName("labelLightNum").getComponent(cc.Label)
        ...
        if (DataManager.CommonData["morrow"] == 0 && btnGet.active == false && DataManager.CommonData["NewUserSgin"]["day"] == 1)
            this.node.runAction(cc.sequence(cc.delayTime(.5), cc.callFunc(() => this.getAward())))
    }
