...
export default class OperatePopupItem extends BaseComponent {
    ...
    @property()
    data:[] = []
    thisComponentName = "OperatePopupItem.hbermj"
    ...
    init(data) {
        ...
        for (const value of vecCards) {
            GameLogic.Instance().addPrefab(this.prefab_card, this.node).getComponent("Card.hbermj").init(value)
        }
    }

    onPressOperate() {
        GameLogic.Instance().gamescene.mahjong_ermj_playcard_ack_sender(this.data.opcode, this.data.value, GameLogic.Instance().gamescene.curSerialID)
    }
}