...
import AudioManager from "./AudioManager.hbermj";

import {MsgBox, gobackToMain, unenoughGold, checkServerMoneyLimit, showShopPop, czcEvent, getGameAward} from "../base/BaseFuncTs";
...
export default class GameLogic extends BaseScene {
    ...
    socketName: string = 'hbermj'

    moduleExt: string = 'hbermj'

    moduleRes: string = 'moduleHBErmjRes/'

    MAX_PLAYER_NUM:number = 2
    ...
    isPrivateRoom() {
        // return (this.serverInfo.lc_room_mode == 1 || this.serverInfo.lc_room_mode == 4)
    ...
    showNewUserSignPop(initParam = []) {
        if(isWeChatGame()){
            SceneManager.Instance.popScene("moduleLobby", "NewUserSignHBPop", initParam)    
        }else{
    ...
    showGameCommonTipLayer(initParam) {       
        initParam = initParam || [] 
        SceneManager.Instance.popScene<String>("moduleHBErmjRes", "GameCommonTipLayer", initParam)
    }
    ...
    showGameFanXingTipLayer(initParam = []) {     
        ...
        SceneManager.Instance.popScene<String>("moduleHBErmjRes", "GameFanXingTipLayer", initParam)
    }
    ...
    showGameResultLayer(initParam = []) {     
        ...
        initParam["zorder"] = this.gamescene.zOrder.nodeJieSuan
        SceneManager.Instance.popScene<String>("moduleHBErmjRes", "GameResultLayer", initParam)
    }
    	...
    	if (this.isPrivateRoom()) {
            if (this.gamescene.isGameStart()) {
                this.showGameCommonTipLayer({
                    msg: "游戏已开始\n需其他玩家确认后才能解散房间~",
                    style: 2,
                    confirmCback: () => { this.gamescene.mahjong_cb_dismiss_private_table_req_sender() },
                    cancelCback: () => { this.LeaveGameScene() },
                })
            } else {
	...
	LeaveGameScene(param = {isOnGameExit: 0}) {
        czcEvent("二人麻将", "离桌1", "游戏返回大厅"+ (DataManager.CommonData["morrow"] <= 1 ? DataManager.CommonData["morrow"] + "天新用户" : "老用户"))
        if(this.henry_debug){
            return
        }
    ...
    showPrivateZhanJiLayer(initParam) {
        ...
        if (!!this.gamescene) {
            ...
            SceneManager.Instance.popScene<String>("moduleHBErmjRes", "PrivateZhanJiLayer", initParam)
        }
    }
}